...
class SourceTreeDiffList
  ...
  def make(a_path, b_path)
    ...
    diff_lines.each_line do |line|
      ...
      puts("##1 #{line}") if @debug > 1
      if (line =~ /^Files\s+(\S+)\s+and\s+(\S+)\s+differ$/) then
        path = $2.gsub(/^#{b_path}\//,'')
        @list.push([path, true , true ])
        puts("##1 #{path} true, true") if @debug > 1
        next
      end
      puts("##2 #{line}") if @debug > 1
      if (line =~ /^Only in #{b_path}\/(.*):\s+(.*)$/) then
        ...
        puts("##2 #{path} false, true") if @debug > 1
        next
      end
      puts("##3 #{line}") if @debug > 1
      if (line =~ /^Only in #{a_path}\/(.*):\s+(.*)$/) then
        ...
        puts("##3 #{path} true , false") if @debug > 1
        next
      end
      puts("##4 #{line}") if @debug > 1
    end
    @a_path = a_path
    @b_path = b_path
